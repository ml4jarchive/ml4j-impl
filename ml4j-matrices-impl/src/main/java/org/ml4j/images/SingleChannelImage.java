...
public class SingleChannelImage extends ImageBase {
		
	private float[] data;
	private int startIndex;
	
	public SingleChannelImage(float[] data, int startIndex, int height, int width, int paddingHeight, int paddingWidth, int examples) {
	...
	public void populateIm2col(float[] data, int startIndex, int filterHeight, int filterWidth, int strideHeight, int strideWidth, int channels) {	
		...
		for (int h =0; h < filterHeight; h++) {
			for (int w =0; w < filterWidth; w++) {
				populateDataSubImage(data, startIndex, h, w, windowHeight, windowWidth, strideHeight, strideWidth, false); 
	...
	public void populateIm2col2(float[] data, int startIndex, int filterHeight, int filterWidth, int strideHeight, int strideWidth, int channels) {	
		...
		for (int h =0; h < filterHeight; h++) {
			for (int w =0; w < filterWidth; w++) {
				populateDataSubImage(data, startIndex, h, w, windowHeight, windowWidth, strideHeight, strideWidth, true); 
	...
	public Image dup() {
		...
		return new SingleChannelImage(dataDup, startIndex, height, width, paddingHeight, paddingWidth, examples);
	}
	
	@Override
	public Image softDup() {
		return new SingleChannelImage(data, startIndex, height, width, paddingHeight, paddingWidth, examples);
	}
