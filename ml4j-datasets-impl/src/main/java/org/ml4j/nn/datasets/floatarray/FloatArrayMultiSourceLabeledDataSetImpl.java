...
public class FloatArrayMultiSourceLabeledDataSetImpl extends MultiSourceLabeledDataSetImpl<float[], float[]>
		implements FloatArrayLabeledDataSet {

	private int featureCount;
	private int labelFeatureCount;

	public FloatArrayMultiSourceLabeledDataSetImpl(DataSet<float[]> dataSet, DataSet<float[]> labelSet,
	...
	public FloatArrayBatchedLabeledDataSet toBatchedLabeledDataSet(int batchSize) {
		...
		return new FloatArrayBatchedLabeledDataSetImpl(() -> StreamUtils.zip(batchedDataSet.stream(),
				batchedLabelSet.stream(), (l, r) -> createLabeledDataBatch(l, r)), featureCount, labelFeatureCount);

	}

	private DataBatch<LabeledData<float[], float[]>> createLabeledDataBatch(DataBatch<float[]> data,
			DataBatch<float[]> labels) {
