...
public abstract class ScaleAndShiftAxonsTestBase {
  ...
  @Before
  public void setUp() {
    matrixFactory = new JBlasMatrixFactory2();
  }
  ...
  public void testPushLeftToRightWithLeftHandBias() {
    ...
    Matrix outputMatrix = axonsActivation.getOutput().getActivations(matrixFactory);
  ...
  public void testPushLeftToRightThenRightToLeft() {
	  ...
	  int featureCount = 10;
	    ...
	    AxonsContext context = new AxonsContextImpl(matrixFactory, false);
	    AxonsActivation axonsActivation = axons.pushLeftToRight(input, null, context);
	    NeuronsActivation rightNeuronsActivation = new NeuronsActivationImpl(axonsActivation.getOutput().getActivations(matrixFactory),
		        NeuronsActivationFeatureOrientation.ROWS_SPAN_FEATURE_SET);
	    
	    AxonsActivation finalActivation = axons.pushRightToLeft(rightNeuronsActivation, axonsActivation, context);
	    Matrix finalOutput = finalActivation.getOutput().getActivations(matrixFactory);
