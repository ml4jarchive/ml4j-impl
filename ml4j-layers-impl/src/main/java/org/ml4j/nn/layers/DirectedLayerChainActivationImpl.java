...
public class DirectedLayerChainActivationImpl
		extends DirectedComponentChainActivationImpl<NeuronsActivation, DirectedLayerActivation>
		implements DirectedLayerChainActivation {
	...
	public DirectedLayerChainActivationImpl(List<DirectedLayerActivation> directedLayerActivations) {
		...
		if (this.getActivations().size() > 1) {
			this.precedingChainActivation = new DirectedLayerChainActivationImpl(
					getActivations(directedLayerActivations).subList(0, directedLayerActivations.size() - 1));
		}
	}
	...
	private static DirectedLayerActivation getFinalActivation(List<DirectedLayerActivation> directedLayerActivations) {
		if (directedLayerActivations.isEmpty()) {
			throw new IllegalArgumentException(
					"At least one instance of DirectedLayerActivation is required in a DirectedLayerChainActivation");
		}
		return directedLayerActivations.get(directedLayerActivations.size() - 1);
	}

	private static List<DirectedLayerActivation> getActivations(
			List<DirectedLayerActivation> directedSynapsesActivations) {
	...
	public DirectedComponentGradient<NeuronsActivation> backPropagate(CostFunctionGradient outerGradient) {
		...
		DirectedComponentGradient<NeuronsActivation> activationFunctionGradient = finalLayerActivation
